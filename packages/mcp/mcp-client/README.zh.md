...
| `serverName` | 两者 | 是 | 该服务器面向模型工具名称的 namespace；`[A-Za-z0-9_-]{1,32}`，在其精确 scope 内唯一 |
...
| `startupTimeoutMs` | 两者 | 否 | 连接并发布初始工具世代的最长时间（毫秒，默认 30000） |
...
- 同一精确 scope 中重复的 `serverName` 会使后加载的插件实例失败。root 和每个 Agent scope 独立保留名称。
- 当 root/global 中已经存在同名公开工具时，Agent-scoped MCP 工具世代会拒绝注册。之后才注册的 root/global 工具仍然有效，并依照 ToolRuntime 的普通 scoped-shadow 优先级被 scoped MCP 工具遮蔽；这是有意保留的注册顺序限制。
- 服务器在工具列表中两次列出同一工具名称时，该列表会作为无效工具列表被拒绝。
- 外部注册抢占该服务器 namespace 时，会回滚整个世代（绝不保留部分集合），并明确报错。

## 行为

- 连接时：插件激活会等待 `listTools()`，并在组合开始首个轮次前通过 `ctx.tools.register()` 以公开名称注册每个工具。`startupTimeoutMs` 默认 30000；发生超时时，激活会等待有界清理完成后再报告启动失败。初始连接、发现或注册失败始终会记录日志；`failOnStartupError: true` 会在回滚该服务器的注册后拒绝激活，默认值 `false` 则仍会激活但不注册工具。
...
- 资源释放会在其有界关闭策略内等待连接尝试与排队的工具同步结算，注销该服务器仍拥有的全部工具，并释放其精确 scope 的 `serverName` 保留。
